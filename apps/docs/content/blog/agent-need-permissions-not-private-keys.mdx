...
You spin up a bot, give it a private key, and let it run.

A trading bot sits there executing strategies with full access to funds. It doesn't need most of that power, but there's no real way to scope it down. So it just... has everything.

Sometimes it's even simpler. A script. A cron job. Something that runs every few minutes and performs a task. And again, it uses the same key. The same level of control as the actual user. And now with AI agents, we're taking it a step further. We're giving systems that are inherently unpredictable direct signing authority. Not limited access. Not scoped permissions. Just a private key and full control.
...
sequenceDiagram
    ...
    Agent->>Account: Sign transaction (private key)
    Account->>Blockchain: Execute ANY action
    Blockchain-->>Agent: Success / Failure
`} />
...
It works. That's the thing, all of these setups work. They get the job done and they're already in production, everywhere.

But if you zoom out, they all rely on the same pattern.
...
That assumption breaks the moment you introduce automation, and the current model just doesn't express that.

### What if accounts weren't tied to keys at all?

Instead of thinking of an account as something controlled by a key, you can think of it as something that enforces rules.

This is the shift that things like [account abstraction](https://ethereum.org/roadmap/account-abstraction/) are pushing toward. Instead of relying on a single private key for every action, accounts can define their own validation logic - how transactions are approved, under what conditions, and by whom.

An account can decide what is allowed, not just who can sign, but:

- what actions are valid
- how much can be spent
- when something is allowed to execute

Instead of a static key, you get a programmable system.

### Permissions instead of ownership
...
And those permissions can be scoped:

- limited to a specific contract
- capped to a certain amount
- valid only for a period of time

This is where concepts like session keys come in.

A [session key](https://docs.erc4337.io/smart-accounts/session-keys-and-delegation.html?utm_source=chatgpt.com) is essentially a delegated signer with constraints. It can execute transactions on behalf of an account but only within predefined limits, like time, value, or allowed actions.

Instead of giving an agent full control, you give it a session key:

- valid for 1 hour
- allowed to trade up to a limit
- restricted to a specific protocol

And importantly, it never has access to the main key.
